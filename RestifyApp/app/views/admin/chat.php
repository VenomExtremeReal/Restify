...
<div style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem;">
            ...
            <?php foreach ($conversations as $conversation): ?>
                <div style="padding: 1rem; border-bottom: 1px solid #eee; cursor: pointer;"
                    ...
                    <small style="color: #999;">
                        <?= isset($conversation['created_at']) ? date('d/m/Y H:i', strtotime($conversation['created_at'])) : '' ?>
                    </small>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    ...
    <div class="card">
        <?php if (!empty($messages)): ?>
            <h3><?= t('chat_with') ?> <?= htmlspecialchars($conversations[0]['restaurant_name'] ?? t('restaurant')) ?></h3>
